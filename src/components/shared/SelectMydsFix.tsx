...
import { CheckCircleFillIcon, ChevronDownFillIcon } from '@govtechmy/myds-react/icon'
...
const isMultiple = (props: SelectProps): props is SelectMultiple => Boolean(props.multiple)
...
const Select: React.ForwardRefExoticComponent<SelectProps> = React.forwardRef((props, ref) => {
  ...
  const defaultVal = isMulti ? props.defaultValue || [] : props.defaultValue || ''
  const [_value, _setValue] = React.useState(defaultVal)
  const [_open, _setOpen] = React.useState<boolean | undefined>(props.open || false)
  ...
  return (
    <SelectContext.Provider
      value={{
        ...props,
        _value: typeof currentValue === 'undefined' ? (isMulti ? [] : '') : currentValue,
...
const SelectValue: React.ForwardRefExoticComponent<SelectValueProps> = React.forwardRef(
    ...
    if (!isMultiple(rootProps))
      return (
        <>
          ...
          <SelectPrimitive.Value ref={ref} {...props} className={clx('text-inherit', className)}>
...
type SelectCounterProps = React.ComponentProps<'span'>

const SelectCounter: React.ForwardRefExoticComponent<SelectCounterProps> = React.forwardRef(
  ({ children, className, ...props }, ref) => {
    const { size } = React.useContext(SelectContext)

    return (
      <span ref={ref} className={clx(select_counter_cva({ size }), className)} {...props}>
        {children}
      </span>
    )
  }
  ...
  ],
  {
    variants: {
      variant: {
        ...
        ghost: ['bg-transparent border border-transparent', 'hover:bg-bg-white-hover'],
      },
      size: {
...
const select_label_cva = cva(['group-data-[disabled]:text-txt-black-disabled'], {
  variants: {
    size: {
      small: 'text-body-sm',
      medium: 'text-body-md',
...
const SelectLabel: React.ForwardRefExoticComponent<React.ComponentProps<'span'>> = React.forwardRef(
  ({ className, ...props }, ref) => {
    const { size, multiple } = React.useContext(SelectContext)
    return <span ref={ref} className={select_label_cva({ size, multiple, className })} {...props} />
  }
)
...
>(({ className, ...props }, ref) => (
  <SelectXPad>
    <SelectPrimitive.Label
      ref={ref}
      className={clx('text-body-xs px-2.5 py-1.5 font-semibold text-gray-400', className)}
      {...props}
    />
  </SelectXPad>
))
...
const SelectHeader: React.ForwardRefExoticComponent<React.ComponentProps<'div'>> = React.forwardRef(
  ({ className, ...props }, ref) => {
    return (
      <SelectXPad
        ref={ref}
...
const SelectFooter: React.ForwardRefExoticComponent<React.ComponentProps<'div'>> = React.forwardRef(
  ({ className, ...props }, ref) => {
    return (
      <SelectXPad
        ref={ref}
