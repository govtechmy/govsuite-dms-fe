...
The container serves the built app via nginx and injects `VITE_*` environment variables at **runtime** (not build time), so the same image can be deployed with different `.env` values without rebuilding — see `docker/entrypoint.sh`.
...
| `.env.example`           | Template for `VITE_*` environment variables                                                                                            |
| `.env`                   | Your actual values (gitignored, created by you)                                                                                        |
...
Then edit `.env` with the correct values for the target environment (API base URL, app name, support email, etc).
...
1. **On a machine with internet/Docker access**, from this repo, run:
   ...
   - Copies `deploy.sh`, `docker-compose.yml`, `.env`, and both tars into `~/Desktop/govsuite-fe-deploy`
...
4. **On the target server**, inside that folder, run:
   ...
   This:
   - Verifies Docker, Docker Compose v2, `.env`, `docker-compose.yml`, and `frontend-<arch>.tar` are all present
